...
selected.value = `${selectedCity.value} ${selectedDistrict.value} ${selectedBank.value}`;
...
export default {
  data() {
    return {
      selectedCity: "서울시", // 기본 시도
      selectedDistrict: "종로구", // 기본 시군구
      selectedBank: "한국은행", // 기본 은행
      keyword: "종로구 은행",
      map: null,
      markers: [],
      infowindow: new kakao.maps.InfoWindow({ zIndex: 1 }),
...
<template>
  <div class="map-wrap">
    ...
    <div id="menu-wrap" class="bg-white">
      ...
      <p>
        <strong
          >{{ selectedCity }} {{ selectedDistrict }} {{ selectedBank }}</strong
        >
        검색 결과입니다.
      </p>
      <ul id="placesList"></ul>
...
export default {
  ...
  methods: {
    ...
    searchPlaces() {
      // 선택된 시도, 시군구, 은행 정보를 활용하여 검색 키워드 생성
      this.keyword = `${this.selectedDistrict} ${this.selectedBank}`;
